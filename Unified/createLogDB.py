...
if check_years:
    ...
    years = [_f for _f in os.popen('ls /eos/cms/store/logs/prod/').read().split('\n') if _f]
...
for year in years:
    ...
    if check_months:
        ...
        months = [_f for _f in os.popen('ls /eos/cms/store/logs/prod/%s/'%(year)).read().split('\n') if _f]
    ...
    for month in months:
        ...
        if specific:
            ...
            workflows = [_f for _f in os.popen('ls /eos/cms/store/logs/prod/%s/%s/WMAgent/'%(year,month)).read().split('\n') if _f]
        ...
        for workflow in workflows:
            ...
            month = "%02d"%int(month)
            tars = [_f for _f in os.popen('ls /eos/cms/store/logs/prod/%s/%s/WMAgent/%s/'%(year,month,workflow)).read().split('\n') if _f]
            ...
            tasks = set([b for b in ['-'.join(tar.replace(workflow,'').split('-',4)[1:4]) for tar in tars] if b.startswith('LogCollect')])
            ...
            print(tasks)
            for task in tasks:
                tars = [_f for _f in os.popen('ls /eos/cms/store/logs/prod/%s/%s/WMAgent/%s/*%s*.tar'%(year,month,workflow,task)).read().split('\n') if _f]
                print(task,len(tars))
                for tar in tars:
                    tar = tar.split('/')[-1]
                    ...
                    logs = [_f for _f in [b.split('/')[-1] for b in os.popen('tar tf %s'%(tarloc)).read().split('\n')] if _f]
                    for log in logs:
                        record = LogRecord(
                            workflow= workflow,
                            logfile = log,
