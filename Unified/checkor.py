    ...
    def check(self):
        ...
        if not all(pass_stats_check.values()):
            ...
            if possible_recoveries == [] or possible_recoveries == None:
                ...
                sendLog('checkor', '%s is not completed, but has nothing to be recovered, passing along ?' % wfo.name,
                        level='critical')
            ...
            json.dumps(percent_completions, indent=2), json.dumps(fractions_pass, indent=2)))
        ...
        if any([(lumi_upper_limit[out] > 0 and events_per_lumi[out] >= lumi_upper_limit[out]) for out in
                events_per_lumi]):
        ...
        for output in wfi.request['OutputDatasets']:
            ...
            all_blocks = set([x[0] for x in rucio_filecount_pb])
            ...
            if len(all_blocks) == len(all_filecount_pb):
                rucio_presence[output] = sum([x[1] for x in all_filecount_pb])
            else:
        ...
        one_output_not_in_rucio = any([Nfiles == 0 for Nfiles in list(rucio_presence.values())])
        ...
        out_worth_checking = [out for out in list(custodial_locations.keys()) if
                              out.split('/')[-1] not in vetoed_custodial_tier]
        size_worth_checking = sum([(getDatasetSize(out) / 1023. if not wfi.isRelval() else 0.) for out in
                                   out_worth_checking])  ## size in TBs of all outputs
        size_worht_going_to_ddm = sum([getDatasetSize(out) / 1023. for out in out_worth_checking if
                                       out.split('/')[-1] in to_ddm_tier])  ## size in TBs of all outputs
        all_relevant_output_are_going_to_tape = all(
            [len(sites) != 0 for sites in [custodial_locations[out] for out in out_worth_checking]])

        show_N_only = 10  ## number of files to include in a report log

        time_point("dbs file count", sub_lap=True)

        if not all([dbs_presence[out] == (dbs_invalid[out] + rucio_presence[out]) for out in
                    wfi.request['OutputDatasets']]) and not options.ignorefiles:
            ...
            if not 'recovering' in assistance_tags:
                ...
                for out in dbs_presence:
                    ...
                    if missing_dbs:
                        wfi.sendLog('checkor',
                                    "These %d files are missing in dbs, or extra in rucio, showing %d only\n%s" % (
                                    len(missing_dbs), show_N_only,
        ...
        if (not stop_duplicate_check) and (is_closing or bypass_checks) and (not all_relevant_output_are_going_to_tape):
            ...
            for output in wfi.request['OutputDatasets']:
                ...
                lumis_with_duplicates[output] = [rl for (rl, files) in list(files_per_rl[output].items()) if len(files) > 1]
            ...
            if is_closing and any(duplications.values()):
                ...
                duplicate_notice += json.dumps(dict([(o, len(badl)) for o, badl in list(lumis_with_duplicates.items())]),
                                               indent=2)
                ...
                for out, bads in list(bad_files.items()):
                    if bads:
                        invalidateFiles(bads)  ## for full removal
                        dbs3Client.setFileStatus(bads,
                                                 newstatus=0)  ## invalidate in dbs in the meantime so that the dataset goes into filemismatch category
                    pass
        ...
        if is_closing and not all_relevant_output_are_going_to_tape:
            ...
            if custodial and size_worth_checking > tape_size_limit:
                wfi.sendLog('checkor', "The total output size (%s TB) is too large for the limit set (%s TB)" % (
                size_worth_checking, tape_size_limit))
            ...
            if custodial:
                for output in out_worth_checking:
                    ...
                    if getDatasetSize(output) / 1023 > tape_size_limit:
                        wfi.sendLog('checkor', "%s output size (%s TB) is too large for the limit set (%s TB)" % (
                        output, out_worth_checking[output], tape_size_limit))
                        assistance_tags.add('bigoutput')
                        custodial = None

            if not custodial:
                print("cannot find a custodial for", wfo.name)
                wfi.sendLog('checkor', "cannot find a custodial for %s probably because of the total output size %d" % (
                wfo.name, size_worth_checking))
                sendLog('checkor', "cannot find a custodial for %s probably because of the total output size %d" % (
                wfo.name, size_worth_checking), level='critical')
                ...
                for output in out_worth_checking:
                    if not len(custodial_locations[output]):
                        if rucio_presence[output] >= 1:
                            ...
                            ## does not look good
                            wfi.sendLog('checkor',
                                        'No file in rucio for %s, not good to add to custodial requests' % output)
                            holding.append(output)
                if not holding:
                    is_closing = True
        ...
        if not all([(dbs_invalid[out] <= int(fraction_invalid * dbs_presence[out])) for out in
                    wfi.request['OutputDatasets']]) and not options.ignoreinvalid:
        ...
        for output in wfi.request['OutputDatasets']:
            ...
            rec['correctLumis'] = int(events_per_lumi[output]) if (
                        events_per_lumi[output] > lumi_upper_limit[output]) else True
        ...
        if is_closing:
            ...
            # filemismatch # there is a dbs/rucio mismatch
            ...
            if go_notify:
                ...
                messages = {
                    'recovery': 'Samples completed with missing statistics:\n%s\n%s ' % ('\n'.join(
                        ['%.2f %% complete for %s' % (percent_completions[output] * 100, output) for output in
                         wfi.request['OutputDatasets']]), perflink),
                    'biglumi': 'Samples completed with large luminosity blocks:\n%s\n%s ' % ('\n'.join(
                        ['%d > %d for %s' % (events_per_lumi[output], lumi_upper_limit[output], output) for output in
                         wfi.request['OutputDatasets'] if (events_per_lumi[output] > lumi_upper_limit[output])]),
                                                                                             splitlink),
                    'duplicates': 'Samples completed with duplicated luminosity blocks:\n%s\n' % ('\n'.join(
                        ['%s' % output for output in wfi.request['OutputDatasets'] if
                         output in duplications and duplications[output]])),
...
if __name__ == "__main__":
    ...
    parser.add_option('--strict', help='Only running workflow that reached completed', action='store_true',
                      default=False)
    parser.add_option('--update', help='Running workflow that have not yet reached completed', action='store_true',
                      default=False)

    parser.add_option('--clear', help='Only the workflow that have reached custodial', action='store_true',
                      default=False)
    parser.add_option('--review', help='Look at the workflows that have already completed and had required actions',
                      action='store_true', default=False)
    parser.add_option('--recovering', help='Look at the workflows that already have on-going acdc', action='store_true',
                      default=False)
    ...
    parser.add_option('--ignoreinvalid', help='Force ignoring high level of invalid files', action='store_true',
                      default=False)
