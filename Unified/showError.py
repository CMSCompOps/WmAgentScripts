...
def parse_one(url, wfn):
    ...
    cache = int(sys.argv[2]) if len(sys.argv)>2 else 0
    ...
    expose_archive_code = {'134':defaultdict(lambda : 10),#seg fault
                           '99109':defaultdict(lambda : 10),#stageout
                           '99303' : defaultdict(lambda : 10),#no pkl report. if you are lucky
                           '60450' : defaultdict(lambda : 10),#new
                           '50513':defaultdict(lambda : 10),#new
                           '8001': defaultdict(lambda : 10),# the usual exception in cmsRun
                           }
    expose_condor_code = {'99109':defaultdict(lambda : 10),#stageout
                          '99303':defaultdict(lambda : 10),#no pkl report
                          '60450':defaultdict(lambda : 10),#new
                          '50513':defaultdict(lambda : 50),#new
                          }
    tasks = err.keys()
    tasks.sort()
    if not tasks:
    ...
    html="<html> <center><h1>%s<br><a href=https://dmytro.web.cern.ch/dmytro/cmsprodmon/workflows.php?prep_id=%s>%s</a></center> <br>Updated on %s (GMT)<hr><br>"%(
        tasks[0].split('/')[1],
        wfi.request['PrepID'],
        wfi.request['PrepID'],
    ...
    if wfi.request['RequestType'] in ['ReReco']:
        html += '<a href=../datalumi/lumi.%s.html>lumis</a><br>'% wfi.request['PrepID']
    for task in tasks:  
        print task
            ...
            for errorcode_s in err[task][exittype]:
                ...
                for site in err[task][exittype][errorcode_s]:
                    ...
                    for sample in err[task][exittype][errorcode_s][site]['samples']:
                        ...
                        if errorcode_s in expose_condor_code and expose_condor_code[errorcode_s][agent]:
                        ...
                        for out in sample['output']:
                            ...
                            if out['type'] == 'logArchive':
                                if errorcode_s in expose_archive_code and expose_archive_code[errorcode_s][agent]:
        ...
        html += "<b>%s</b> is missing <b>%s events</b>"%(task.split('/')[-1], miss)
        ...
        for site in all_sites:
            ...
            for code in sorted(all_codes):
                if error_site_count[code][site]:
                    ...
                    html += '<td %s width=200>%d, <font color=%s>&uarr; %.1f%%</font>, <font color=%s>&rarr; %.1f%%</font></td>'% (color,
