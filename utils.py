...
def _searchLog(q, actor, limit, conn, prefix, h=None):
    goodquery = {"query": {"bool": {"must": [{"wildcard": {"meta": "*%s*" % q}}]}}, "sort": [{"timestamp": "desc"}],
                 "_source": ["text", "subject", "date", "meta"]}
...
class UnifiedLock:
    ...
    def deadlock(self):
        ...
        from assignSession import session, LockOfLock
        to_remove = []
        for ll in session.query(LockOfLock).filter(LockOfLock.lock == True).filter(
                LockOfLock.owner.contains(host)).all():
            print(ll.owner)
            try:
...
class StartStopInfo:
    ...
    def get(self, component, metric='lap'):
        res = [oo[metric] for oo in sorted(self.db.find({'component': component}), key=lambda o: o['start']) if
               metric in oo]
        return res
...
class componentCheck(threading.Thread):
    ...
    def check_dbs(self):
        ...
        if 'testbed' in dbs_url:
            blocks = dbsapi.listBlockSummaries(
                dataset='/QDTojWinc_NC_M-1200_TuneZ2star_8TeV-madgraph/Summer12pLHE-DMWM_Validation_DONOTDELETE_Alan_TEST-v1/GEN',
                detail \
                    =True)
        else:
            blocks = dbsapi.listBlockSummaries(
                dataset='/TTJets_mtop1695_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8/RunIIWinter15GS-MCRUN2_71_V1-v1/GEN-SIM',
                detail=True)
        if not blocks:
...
def read_file(target):
    ...
    if target.endswith('json'):
        if is_json(content):
            ...
            sendLog("eosRead", "Error reading json file {} at {}".format(target, datetime.datetime.now()),
                    level='critical')
            return "{}"
    else:
        return content
    ...
    def close(self):
        ...
        while T < self.trials:
            ...
            try:
                ...
                r = os.system(
                    "env EOS_MGM_URL=root://eoscms.cern.ch eos cp %s %s" % (self.cache_filename, self.eos_filename))
                ...
                if bail_and_email:
                    ...
                    print('eos is acting up on %s on %s. not able to copy %s to eos code %s' % (
                    h, time.asctime(), self.eos_filename, r))
                    break
                ...
                if bail_and_email:
                    ...
                    h, time.asctime(), self.eos_filename, str(e)))
                    break
                else:
                    time.sleep(30)
        h = socket.gethostname()
                ...
                if 'SiteBlacklist' in self.campaigns[c]['parameters']:
                    for black in copy.deepcopy(self.campaigns[c]['parameters']['SiteBlacklist']):
                        if black.endswith('*'):
                            ...
                            reg = black[0:-1]
                            self.campaigns[c]['parameters']['SiteBlacklist'].extend(
                                [site for site in (SI.all_sites) if site.startswith(reg)])
    ...
    def check(self, hours_before_kill=24):
        ...
        for lock in locks:
            ...
            if on_since > (hours_before_kill * 60 * 60):
                alarm = "process %s on %s for module %s is running since %s : killing" % (
                pid, host, lock.get('component', None), display_time(on_since))
...
'""" % (str(ssb_metric), str(path_name), str(TIMESTAMP))
    result = json.loads(os.popen(
        ...
        conf["url"], conf["token"], query2)).read())["responses"][0]["hits"]["hits"]
...
class ThreadHandler(threading.Thread):
    ...
    def _run(self):
        ...
        print("[%s] Processing %d threads with %d max concurrent and timeout %s [min]" % (
        self.label, ntotal, self.n_threads, self.timeout))
        ...
        while self.threads:
            if self.timeout and (time.mktime(time.gmtime()) - start_now) > (self.timeout * 60.):
                print("[%s] Stopping to start threads because the time out is over %s " % (
                self.label, time.asctime(time.gmtime())))
        ...
        while sum([t.is_alive() for t in self.r_threads]):
            ...
            if self.timeout and (now - start_now) > (self.timeout * 60.):
                print("[%s] Stopping to start threads because the time out is over %d " % (
                self.label, time.asctime(time.gmtime())))
                return
...
class docCache:
    def __init__(self):
        ...
        self.cache['gwmsmon_totals'] = {
            ...
            'timestamp': time.mktime(time.gmtime()),
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s https://cms-gwmsmon.cern.ch/poolview/json/totals').read()),
            'cachefile': None,
            'default': {}
        }
        self.cache['mcore_ready'] = {
            ...
            'timestamp': time.mktime(time.gmtime()),
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl --retry 5 -s http://cmsgwms-frontend-global.cern.ch/vofrontend/stage/mcore_siteinfo.json').read()),
            'cachefile': None,
            'default': {}
        }
        self.cache['gwmsmon_prod_site_summary'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s https://cms-gwmsmon.cern.ch/prodview//json/site_summary').read()),
            'cachefile': None,
            'default': {}
        }
        self.cache['gwmsmon_prod_maxused'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s https://cms-gwmsmon.cern.ch/prodview//json/maxusedcpus').read()),
            'cachefile': None,
            'default': {}
        }
        self.cache['detox_sites'] = {
            ...
            'timestamp': time.mktime(time.gmtime()),
            'expiration': default_expiration(),
            'getter': lambda: os.popen(
                'curl --retry 5 -s http://t3serv001.mit.edu/~cmsprod/IntelROCCS/Detox/SitesInfo.txt').read().split(
                '\n'),
            'cachefile': None,
            'default': ""
        }
        ...
        def get_invalidation():
            import csv
            TMDB_invalid = set([row[3] for row in csv.reader(os.popen(
...
class siteInfo:
    def __init__(self, override_good=None):
        ...
        self.sites_T1s = [s for s in self.sites_ready if
                          s.startswith('T1_')]  # or s.startswith('T0_'))] ## put the T0 in the T1 : who cares
        ...
        for (site, info) in list(_info_by_site.items()):
            ...
            if key_for_cpu in info and site in self.cpu_pledges and info[key_for_cpu]:
                if self.cpu_pledges[site] < info[key_for_cpu]:
                    if talk: print(site, "could use", info[key_for_cpu], "instead of", self.cpu_pledges[site],
                                   "for CPU")
                    self.cpu_pledges[site] = int(info[key_for_cpu])
                elif self.cpu_pledges[site] > 1.5 * info[key_for_cpu]:
                    if talk: print(site, "could correct", info[key_for_cpu], "instead of", self.cpu_pledges[site],
                                   "for CPU")
                    self.cpu_pledges[site] = int(info[key_for_cpu])
    ...
    def sync(self, site=None):
        if not site:
            ...
            try:
                ...
                try:
                    for fn in [_f for _f in
                               os.popen('ls -1 %s/remaining_*.json | sort ' % monitor_dir).read().split('\n') if _f]:
...
<th>Priority</th>
    ...
    def one_line(self, wf, wfo, count):
        ...
        for io, out in enumerate([o[0] for o in o_and_c]):
            ...
            wf_text += ', <a href="https://cms-logbook.cern.ch/elog/Workflow+processing/?mode=full&reverse=0&reverse=1&npp=20&subtext=%s&sall=q" target="_blank">elog</a>' % (
                pid)
            wf_text += ', <a href=https://%s/couchdb/workloadsummary/_design/WorkloadSummary/_show/histogramByWorkflow/%s>perf</a>' % (
            reqmgr_url, wf)
            wf_text += ', <a href=assistance.html#%s>%s</a>' % (wf, wfo.status)
            wf_text += '<br>'
            wf_text += '<a href="https://cms-unified.web.cern.ch/cms-unified/showlog/?search=%s" target="_blank">history</a>' % (
                pid)
            wf_text += ', <a href=https://dmytro.web.cern.ch/dmytro/cmsprodmon/workflows.php?prep_id=%s>%s</a>' % (
            tpid, tpid)
            ...
            wf_text += ', <a href="https://its.cern.ch/jira/issues/?jql=(text~%s OR text~task_%s ) AND project = CMSCOMPPR" target="_blank">jira</a>' % (
            pid, pid)
            wf_text += '</td>'
            ...
            for f in order:
                ...
                if f == 'acdc':
                    text += '<td><a href=https://%s/reqmgr2/data/request?prep_id=%s&detail=false>%s</a></td>' % (
                    reqmgr_url, tpid, value)
                elif f == 'percentage':
                    ...
                    text += '<td>%s/%s</td>' % (display_N(self.record[wf]['datasets'][out].get('producedN', 'NA')),
                                                display_N(self.record[wf]['datasets'][out].get('expectedN', 'NA')))
                elif f == 'lumis':
                    text += '<td>%s/%s</td>' % (display_N(self.record[wf]['datasets'][out].get('producedL', 'NA')),
                                                display_N(self.record[wf]['datasets'][out].get('expectedL', 'NA')))
                else:
                    text += '<td>%s</td>' % value
    ...
    def summary(self):
        ...
        html.write('<html>')
        html.write('Last update on %s(CET), %s(GMT), <a href=logs/checkor/ target=_blank> logs</a> <br><br>' % (
    ...
    def save(self):
        ...
        os.system('env EOS_MGM_URL=root://eoscms.cern.ch eos cp %s/closedout.json %s/closedout.json.last' % (
        new_base_eos_dir, new_base_eos_dir))
...
def try_getDatasetPresence(url, dataset, complete='y', only_blocks=None, group=None, vetoes=None, within_sites=None):
    ...
    for (site, blocks) in list(locations.items()):
        site_size = sum([block['file_size'] for block in all_blocks if
                         (block['block_name'] in blocks and block['block_name'] in all_block_names)])
...
def getDatasetEventsAndLumis(dataset, blocks=None):
    try:
        ...
        try:
            ...
            sendLog('getDatasetEventsAndLumis',
                    'fatal execption in processing getDatasetEventsAndLumis for %s \n%s' % (dataset, str(e)),
                    level='critical')
            r = 0, 0
    return r
...
def getFilesWithLumiInRun(dataset, run):
    ...
    try:
        reply = dbsapi.listFiles(dataset=dataset, detail=True, run_num=run,
                                 validFileOnly=1) if run != 1 else dbsapi.listFiles(dataset=dataset, detail=True,
                                                                                    validFileOnly=1)
    except:
        try:
            reply = dbsapi.listFiles(dataset=dataset, detail=True, run_num=run,
            ...
            sendLog('getFilesWithLumiInRun', 'Fatal exception in running dbsapi.listFiles for %s %s ' % (dataset, run),
                    level='critical')
            reply = []
    ...
    while True:
        ...
        rreply.extend(
            dbsapi.listFileLumiArray(logical_file_name=these, run_num=run) if run != 1 else dbsapi.listFileLumiArray(
                logical_file_name=these))
        start += bucket
    return rreply
...
def getDatasetLumisAndFiles(dataset, runs=None, lumilist=None, with_cache=False, force=False,
                            check_with_invalid_files_too=False):
    ...
    if runs:
        ...
        files_json = dict([(tuple(map(int, k.split(":"))), v) for (k, v) in list(files_per_lumi.items()) if
                           int(k.split(':')[0]) in runs])
    elif lumilist:
...
def invalidateFiles(files):
    all_OK = True
    conn = http.client.HTTPSConnection('dynamo.mit.edu', cert_file=os.getenv('X509_USER_PROXY'),
                                       key_file=os.getenv('X509_USER_PROXY'))
    for fn in files:
...
def _invalidate(url, wfi, only_resub=False, with_output=True):
    ...
    for fwl in familly:
        ...
        check.append(reqMgrClient.invalidateWorkflow(url, fwl['RequestName'], current_status=fwl['RequestStatus'],
                                                     cascade=False))
...
class agentInfo:
    ...
    def getStatus(self):
        ...
        def drained(stats):
            ...
            if upload.get('dbs_notuploaded', 1) == 0 and upload.get('dbs_open_blocks', 1) == 0 and upload.get(
                    'phedex_notuploaded', 1) == 0 and condor.get('idle', 1) == 0 and condor.get('running', 1) == 0:
                return True
            return False
    ...
    def poll(self, wake_up_draining=False, acting=False, verbose=False):
        ...
        m_rel_num = [(r, [int(frag.replace('patch', '')) for frag in r.split('.')]) for r in
                     list(self.m_release.keys())]
        ...
        for agent, ainfo in list(all_agents.items()):
            ...
            if agent_name in drainings:
                ...
                if (cp <= cpu_running * self.speed_draining_fraction) and (r <= t * self.speed_draining_fraction) and (
                        cr <= cpu_running * self.speed_draining_fraction):
            ...
            if agent_name in runnings:
                ...
                elif not agent_name in self.m_release[
                    top_m_release]:  ## use the major release number to select agents to drain: i.e. ignore patches versions.
        ...
        if not (one_recent_running or one_recent_draining or one_recent_standby):
            ...
            if over_pending:
                msg = 'There is more than %d cpus pending, %d. We need to set an agent aside.' % (
                over_cpus, cpu_pending)
                sendLog('agentInfo', msg, level='critical')
                sendEmail('agentInfo', msg)
            if release_deploy:
                msg = 'There is a new agent release in town %s. Starting to drain other agents from %s' % (
                top_release, sorted(candidates_to_drain))
                sendLog('agentInfo', msg, level='critical')
            ...
            print(
                "An agent was recently put in running/draining/standby. Cannot do any further acting for another %s last running or %s last standby" % (
                display_time(timeout_action),
        ...
        if need_one:
            ...
            if not pick_from:
                if wake_up_draining:
                    ...
                    ## order by the metric
                    pick_from = [o[0] for o in
                                 sorted([(a, m) for (a, m) in wake_up_metric if a in pick_from], key=lambda o: o[1],
                                        reverse=True)]
...
class workflowInfo:
    ...
    def isGoodToConvertToStepChain(self, keywords=None, talk=False, debug=False):
        ...
        good = self.request[
                   'RequestType'] == 'TaskChain' and more_than_one_task and found_in_transform_keywords and single_tiers and (
                           all_same_cores or acceptable_efficiency) and output_from_single_task and all_same_arch
        ...
        if good:
            if isHighPriority:
                print(("High priority workflow %s is okay to be converted to stepchain with efficiency %s" % (
    ...
    def getWMErrors(self, cache=0):
        try:
            ...
            self.conn = make_x509_conn(self.url)
            r1 = self.conn.request("GET", '/wmstatsserver/data/jobdetail/%s' % (self.request['RequestName']),
                                   headers={"Accept": "*/*"})
            r2 = self.conn.getresponse()
        ...
        if cache:
            if os.path.isfile(f_cache):
                ...
                if (now - stamp) < cache:
                    ...
                    return self.wmstats
        r1 = self.conn.request("GET", '/wmstatsserver/data/request/%s' % self.request['RequestName'],
        ...
        for f in all_files:
            if not f.startswith('/store/unmerged/') and not f.startswith('MCFakeFile-'):
                if f in file_block_cache:
                    ...
                    if file_block:
                        for _f in getDatasetFileArray(file_block.split('#')[0], detail=True,
    ...
    def getRecoveryDoc(self, collection_name=None):
        ...
        try:
            ...
            r1 = self.conn.request("GET",
                                   '/couchdb/acdcserver/_design/ACDC/_view/byCollectionName?key="%s"&include_docs=true&reduce=false' % collection_name)
            r2 = self.conn.getresponse()
    ...
    def getPileUpJSON(self, task):
        ...
        for agent in agents:
            ...
            src = '%s:/data/srv/wmagent/current/install/wmagent/WorkQueueManager/cache/%s/WMSandbox/%s/cmsRun1/pileupconf.json' % (
            agent, wf, task)
    ...
    def getWorkQueue(self):
        if not self.workqueue:
            ...
            while TT < 5:
                ...
                try:
                    r1 = self.conn.request("GET",
                                           '/couchdb/workqueue/_design/WorkQueue/_view/elementsByParent?key="%s"&include_docs=true' %
                                           self.request['RequestName'])
                    r2 = self.conn.getresponse()
    ...
    def getGlideMon(self):
        try:
            gmon = json.loads(os.popen(
                'curl -s https://cms-gwmsmon.cern.ch/prodview/json/%s/summary' % self.request['RequestName']).read())
            return gmon
        except:
    ...
    def getSiteWhiteList(self, pickone=False, verbose=True):
        ...
        if blow_up > max_blow_up:
            ...
            new_sites_allowed = list(
                set(sites_allowed) & set([site for site in sites_allowed if SI.cpu_pledges[site] > needed_cores]))
            if new_sites_allowed:
                sites_allowed = new_sites_allowed
                print("swaping", verbose)
                if verbose:
                    print("restricting site white list because of blow-up factor", min_child_job_per_event,
                          root_job_per_event, max_blow_up)
        ...
        for campaign in self.getCampaigns():
            ...
            if c_sites_allowed:
                if verbose:
                    print("Using site whitelist restriction by campaign,", campaign, "configuration",
                          sorted(c_sites_allowed))
    ...
    def checkSplitting(self):
        ...
        if self.request['RequestType'] == 'StepChain':
            ...
            for spl in splits:
                ...
                if avg_events_per_job and sizeperevent and (avg_events_per_job * sizeperevent) > (
                        GB_space_limit * 1024. ** 2):
                    print(
                        "The output size of task %s is expected to be large : %d x %.2f kB = %.2f GB > %f GB " % (tname,
                                                                                                                  avg_events_per_job,
                                                                                                                  sizeperevent,
                ...
                if events_per_lumi and 'events_per_job' in task:
                    ...
                    if timeperevent:
                        ...
                        if (time_per_input_lumi > (job_timeout * 60 * 60)):  ##45h
                            ...
                            msg = "The running time of task {} is expected to be too large even for one lumi section: {} x {:.2f} s = {:.2f} h > {} h. Should go as low as {}".format(
                                tname,
                    ...
                    if sizeperevent:  # and (avg_events_per_job * sizeperevent ) > (GB_space_limit*1024.**2):
                        ...
                        this_max_events_per_lumi = int((
                                                                   GB_space_limit * 1024. ** 2) / sizeperevent / efficiency_factor / filter_efficiency_at_this_task)
                        # This is the possible events per lumi for the whole chain, before taking any filter efficiency into account

                        this_max_events_per_job = int(
                            (GB_space_limit * 1024. ** 2) / sizeperevent / filter_efficiency_at_this_task)
                        ...
                        if (size_per_input_lumi > (GB_space_limit * 1024. ** 2)):
                            ...
                            msg = "The output size task %s is expected to be too large : %.2f GB > %f GB even for one lumi (effective lumi size is ~%d), should go as low as %d" % (
                            tname,
                            ...
                            max_events_per_lumi.append(
                                this_max_events_per_lumi)  ## adding this to that later on we can check and adapt the split 0

                        elif (avg_events_per_job * sizeperevent * filter_efficiency_at_this_task) > (
                                GB_space_limit * 1024. ** 2):
                            ...
                            msg = "The output size of task %s is expected to be too large : %d x %.2f kB * %.4f = %.2f GB > %f GB. Reducing to %d " % (
                            tname,
                            ...
                            self.sendLog('assignor',
                                         msg)  # We don't hold the workflow here so no need to send critical message
                    ...
                    if max_events_per_lumi:
                        effective_output_lumi_at_this_task = min(events_per_lumi_at_this_task, min(
                            max_events_per_lumi)) * filter_efficiency_at_this_task

                    if (effective_output_lumi_at_this_task < min_lumi) and (not prim) and (not self.isRelval()) and (
                    not small_lumi):  # Only do this once per workflow
                        msg = "{} will get {} events per lumi in output. Smaller than {} is troublesome.".format(tname,
                                                                                                                 effective_output_lumi_at_this_task,
                                                                                                                 min_lumi)
                        self.sendLog('assignor', msg)
            ...
            if max_events_per_lumi:
                if events_per_lumi_inputs:
                    if min(max_events_per_lumi) < events_per_lumi_inputs:
                        ...
                        msg = "Possible events per lumi of this wf (min(%s)) is smaller than %s evt/lumi of the input dataset" % (
                        max_events_per_lumi, events_per_lumi_inputs)
                        self.sendLog('assignor', msg)
                        critical_msg = msg + '\nWorkflow URL: https://dmytro.web.cern.ch/dmytro/cmsprodmon/workflows.php?prep_id=task_{}'.format(
                            self.getPrepIDs()[0])
                        sendLog('assignor', critical_msg, level='critical')
                        ...
                        print("The smallest value of %s is ok compared to %s evt/lumi in the input" % (
                        max_events_per_lumi, events_per_lumi_inputs))
                else:
                    root_split = splits[0]
    ...
    def getSchema(self):
        self.conn = make_x509_conn(self.url)
        r1 = self.conn.request("GET", '/reqmgr2/data/request?name=%s' % self.request['RequestName'],
                               headers={"Accept": "application/json"})
        r2 = self.conn.getresponse()
    ...
    def _taskDescending(self, node, select=None):
        ...
        if (not select):  # or (select and node.taskType == select):
            ...
            for (key, value) in list(select.items()):
                if (type(value) == list and getattr(node, key) in value) or (
                        type(value) != list and getattr(node, key) == value):
                    all_tasks.append(node)
                    break
        ...
        for output in self.request['OutputDatasets']:
            ...
            if output_event_expected and with_event:
                ...
                if e_fraction > percent_completions[output]:
                    ...
                    self.sendLog(caller, "overiding : event completion real %s expected %s for %s" % (
                    event_count, output_event_expected, output))
    ...
    def getOutputPerTask(self):
        ...
        for t in self.getWorkTasks():
            parse_what = t.subscriptions.outputModules if hasattr(t.subscriptions,
                                                                  'outputModules') else t.subscriptions.outputSubs
            if parse_what:
    ...
    def getSplittingsNew(self, strip=False, all_tasks=False):
        self.conn = make_x509_conn(self.url)
        r1 = self.conn.request("GET", '/reqmgr2/data/splitting/%s' % self.request['RequestName'],
                               headers={"Accept": "application/json"})
        r2 = self.conn.getresponse()
        ...
        for spl in result:
            ...
            if strip:
                for drop in ['algorithm', 'trustPUSitelists', 'trustSitelists', 'deterministicPileup', 'type',
                             'include_parents', 'lheInputFiles', 'runWhitelist', 'runBlacklist', 'collectionName',
                             'group', 'couchDB', 'couchURL', 'owner', 'initial_lfn_counter', 'filesetName', 'runs',
                             'lumis']:
    ...
    def getSplittings(self):
        ...
        for task in self.getWorkTasks():
            ...
            get_those = ['events_per_lumi', 'events_per_job', 'lumis_per_job', 'halt_job_on_file_boundaries',
                         'max_events_per_lumi',
                         'halt_job_on_file_boundaries_event_aware']  # ,'couchdDB']#,'couchURL']#,'filesetName']
            translate = {
        ...
        while '%s%d' % (base, t) in self.request:
            if member in self.request['%s%d' % (base, t)]:
                if func:
                    ...
                    coll[self.request['%s%d' % (base, t)]['%sName' % base]] = self.request['%s%d' % (base, t)].get(
                        member, default)
            t += 1
        return coll
        ...
        def invertDigits(st):
            if st[0].isdigit():
                ...
                while st[0].isdigit():
                    number += st[0]
                    st = st[1:]
                isolated = [(st[i - 1].isupper() and st[i].isupper() and st[i + 1].islower()) for i in
                            range(1, len(st) - 1)]
                if any(isolated):
...
def getFailedJobs(taskname, caller='getFailedJobs'):
    ...
    try:
        r1 = conn.request("GET",
                          '/wmstatsserver/data/filtered_requests?RequestName=%s&mask=PrepID&mask=AgentJobInfo' % (
                              wfname), headers={"Accept": "application/json"})
        r2 = conn.getresponse()
...
def aggregateListFileLumis(response):
    ...
    for key, value in tmpDict.items():
        aggregatedResponse.append(
            {'run_num': key[0], 'lumi_section_num': value["lumi_section_num"], 'logical_file_name': key[1],
